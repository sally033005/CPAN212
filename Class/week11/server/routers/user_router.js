...
router.post("/login", (req, res) => {
  // 1 - parse the form information
  const { email, password } = req.body;

  User.findOne({ email: email }) // 2 - Find user account by email
    .then((user_account) => {
      if (!user_account) {
      ...
      bcrypt
        ...
        .then((isMatch) => {
          ...
          // we know at this point that a user exists and their information was inputted correctly
          req.session.user = {
